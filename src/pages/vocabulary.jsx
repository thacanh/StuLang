...
import { ChevronRight, Volume2, Filter, ChevronLeft, ChevronRightIcon, Check, Plus } from "lucide-react"
...
export default function VocabularyPage() {
  ...
  const handleAddToCycle = async (wordId) => {
    try {
      ...
      let errorMessage = "Không thể thêm từ vựng vào chu kỳ"
      if (err.response?.status === 404) {
        errorMessage = "Bạn chưa có chu kỳ học nào. Hãy tạo chu kỳ học mới!"
      } else if (err.response?.status === 400) {
        errorMessage = "Từ vựng đã có trong chu kỳ học"
      }
